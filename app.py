...
st.markdown(
    ...
    .f1-grid-4 { grid-template-columns:repeat(4, minmax(0, 1fr)); }
    .f1-chart-card { border:1px solid rgba(255,255,255,.08); border-radius:16px; background:rgba(7,12,20,.72); padding:.65rem; margin:.65rem 0; }
    .f1-insight { border-left:4px solid #ff383f; border-radius:12px; background:rgba(255,255,255,.045); padding:.75rem .8rem; margin:.45rem 0; }
    .f1-insight b { color:#fff; }
    ...
    @media (max-width:760px) {
      ...
      .f1-grid-5, .f1-grid-4, .f1-grid-3 { grid-template-columns:repeat(2, minmax(0, 1fr)); }
    }
...
def plot_shift_map_compare(
    ...
    """RPM-vs-speed shift map using a broad telemetry sample.

    This intentionally keeps most of the data cloud visible. Public FastF1 data is
    noisy in some zones, so the scatter is made small and transparent rather than
    over-filtered. Linear trend lines are inferred per gear as a ratio proxy.
    """
    fig = go.Figure()
    gear_colors = {
        1: "#ff2d55",
    ...
    def _prep(driver: str) -> pd.DataFrame:
        ...
        required = {"Speed", "RPM", "nGear"}
        if tel.empty or not required.issubset(tel.columns):
            return pd.DataFrame()

        cols = ["Speed", "RPM", "nGear"]
        if "Throttle" in tel.columns:
            cols.append("Throttle")
        if "Brake" in tel.columns:
            cols.append("Brake")
        if "Distance" in tel.columns:
            cols.append("Distance")
        df = tel[cols].dropna(subset=["Speed", "RPM", "nGear"]).copy()
        if df.empty:
            return df

        df["Gear"] = df["nGear"].round().astype(int)
        df = df[df["Gear"].between(1, 8)]
        df = df[(df["Speed"] > 25) & (df["RPM"] > 4500)]

        # Remove only the most obvious braking/coasting clutter, but keep much
        # more data than the WOT-filtered version. If throttle is unavailable,
        # use all valid samples.
        if "Throttle" in df.columns:
            df = df[df["Throttle"] >= 35]
        if "Brake" in df.columns:
            df = df[~df["Brake"].astype(bool)]
        return df.sort_values("Speed").copy()

    def _trend(g: pd.DataFrame) -> Optional[Tuple[float, float, np.ndarray, np.ndarray]]:
        if len(g) < 8:
            return None
        x = g["Speed"].to_numpy(dtype=float)
        y = g["RPM"].to_numpy(dtype=float)
        if np.nanmax(x) - np.nanmin(x) < 8:
            return None

        # Use percentile trimming for the fit only. Scatter still shows the
        # broader data cloud.
        lo_x, hi_x = np.nanpercentile(x, [2, 98])
        lo_y, hi_y = np.nanpercentile(y, [2, 98])
        keep = (x >= lo_x) & (x <= hi_x) & (y >= lo_y) & (y <= hi_y)
        x_fit, y_fit = x[keep], y[keep]
        if len(x_fit) < 8:
            x_fit, y_fit = x, y
        slope, intercept = np.polyfit(x_fit, y_fit, 1)
        x_line = np.linspace(np.nanpercentile(x_fit, 3), np.nanpercentile(x_fit, 97), 32)
        y_line = slope * x_line + intercept
        return float(slope), float(intercept), x_line, y_line

    driver_specs = [(driver_1, "circle", "solid", 0.22, "Driver 1")]
    if driver_2 and driver_2 != driver_1:
        driver_specs.append((driver_2, "diamond", "dash", 0.20, "Driver 2"))
    ...
    for idx, (driver, symbol, dash, opacity, driver_text) in enumerate(driver_specs):
        ...
        fig.add_trace(go.Scatter(
            ...
            marker=dict(symbol=symbol, size=7, color="rgba(245,247,251,0.75)"),
            line=dict(color="rgba(245,247,251,0.75)", width=1.0, dash=dash),
            showlegend=True,
        ))
    ...
    for driver, marker_symbol, dash_style, opacity, driver_text in driver_specs:
        ...
        for gear in range(1, 9):
            ...
            hover_cols = ["Distance"] if "Distance" in g.columns else []
            ...
            fig.add_trace(go.Scatter(
                ...
                marker=dict(
                    ...
                    size=3.2 if marker_symbol == "circle" else 4.0,
                    symbol=marker_symbol,
                    opacity=opacity,
                    line=dict(width=0),
                ),
                customdata=g[hover_cols] if hover_cols else None,
                hovertemplate=(
            ...
            if trend is not None:
                ...
                fig.add_trace(go.Scatter(
                    ...
                    showlegend=False,
                    line=dict(color=gear_colors[gear], width=1.05, dash=dash_style),
                    opacity=0.88 if dash_style == "solid" else 0.65,
    ...
    fig.add_annotation(
        ...
        text="Broad telemetry sample. Dots/solid = driver 1; diamonds/dashed = driver 2. Thin transparent points keep the full shift pattern visible.",
...
def plot_lab_figure(lap_tels: Dict[str, pd.DataFrame], labels: Dict[str, str], styles: Dict[str, Dict[str, str]], drivers: List[str], x_axis: str, y_channels: List[str], smoothing: int = 1, line_mode: str = "lines") -> go.Figure:
    fig = go.Figure()
    if not drivers or not y_channels:
        return update_fig_layout(fig, height=420, ytitle="")
    for channel in y_channels:
        for drv in drivers:
            tel = lap_tels.get(drv, pd.DataFrame())
            if tel.empty or channel not in tel.columns:
                continue
            df = tel.copy()
            if x_axis == "LapPercent":
                if "Distance" not in df.columns or df["Distance"].max() <= 0:
                    continue
                df["LapPercent"] = 100.0 * df["Distance"] / df["Distance"].max()
                xcol = "LapPercent"
            elif x_axis in df.columns:
                xcol = x_axis
            else:
                continue
            y = pd.to_numeric(df[channel], errors="coerce")
            if channel == "Brake":
                y = y.astype(float) * 100.0 if y.max() <= 1 else y
            if smoothing and smoothing > 1:
                y = y.rolling(int(smoothing), min_periods=1, center=True).mean()
            fig.add_trace(go.Scatter(
                x=df[xcol], y=y, mode=line_mode,
                name=f"{drv} · {channel}",
                line=dict(color=styles.get(drv, {}).get("color", "#aaa"), width=2),
                marker=dict(size=4, opacity=.55),
                legendgroup=drv,
                hovertemplate=f"{drv}<br>{channel}=%{{y:.3f}}<br>{x_axis}=%{{x:.2f}}<extra></extra>",
            ))
    fig.update_layout(
        height=460, margin=dict(l=18, r=18, t=35, b=35),
        xaxis_title=x_axis.replace("TimeSeconds", "Time [s]").replace("Distance", "Distance [m]").replace("LapPercent", "Lap [%]"),
        yaxis_title="Selected channel value",
        hovermode="x unified", legend=dict(orientation="h", y=1.08)
    )
    return fig


def build_insight_rows(selected_laps: Dict[str, pd.Series], reference_driver: str, deltas: List[pd.DataFrame], exit_table: pd.DataFrame, stint_export: pd.DataFrame) -> List[str]:
    insights = []
    ...
    for drv, lap in selected_laps.items():
        if drv == reference_driver or ref_time is None:
            continue
        ...
        insights.append(f"<div class='f1-insight'><b>{html.escape(drv)}</b> is <b>{abs(diff):.3f}s {direction}</b> than reference {html.escape(reference_driver)} on the selected lap.</div>")
    if deltas:
        for ddf in deltas:
            if ddf.empty:
                continue
            drv = str(ddf["Driver"].iloc[0])
            worst = ddf.loc[ddf["DeltaSeconds"].idxmax()]
            best = ddf.loc[ddf["DeltaSeconds"].idxmin()]
            insights.append(f"<div class='f1-insight'><b>{html.escape(drv)}</b>: largest loss vs reference around <b>{worst['Distance']:.0f} m</b> ({worst['DeltaSeconds']:+.3f}s); largest gain around <b>{best['Distance']:.0f} m</b> ({best['DeltaSeconds']:+.3f}s).</div>")
    if not exit_table.empty and "MeanAccel_ms2" in exit_table.columns:
        best = exit_table.sort_values("MeanAccel_ms2", ascending=False).iloc[0]
        insights.append(f"<div class='f1-insight'>Best inferred corner-exit acceleration sample: <b>{html.escape(str(best['Driver']))}</b> in <b>{html.escape(str(best['Zone']))}</b>, {best['MeanAccel_ms2']:.2f} m/s² mean.</div>")
    if stint_export is not None and not stint_export.empty:
        long_runs = stint_export[stint_export["RunType"].astype(str).str.contains("Long run", na=False)]
        if not long_runs.empty:
            row = long_runs.sort_values("MedianLap_s").iloc[0]
            insights.append(f"<div class='f1-insight'>Best long-run candidate: <b>{html.escape(str(row['Driver']))}</b>, stint {row['Stint']}, median <b>{row['MedianLap_s']:.3f}s</b> over {int(row['TimedLapsUsed'])} timed laps on {html.escape(str(row['Compound']))}.</div>")
    if not insights:
        insights.append("<div class='f1-insight'>Load at least two drivers and a valid lap selection to generate comparative insights.</div>")
    return insights
...
with tabs[2]:
    st.markdown("### Flexible plot lab")
    st.caption("Overlay any available telemetry channels against distance, time, lap percentage or speed. This preserves the standard plots while giving you a Concerto/ATLAS-style scratch plot area.")
    available_channels = sorted({c for tel in lap_tels.values() for c in tel.columns if c in ["Speed", "Throttle", "Brake", "nGear", "RPM", "DRS", "Accel_ms2", "Accel_g", "Speed_ms"]})
    pc1, pc2 = st.columns([1, 1])
    with pc1:
        plot_drivers = st.multiselect("Drivers in plot", list(selected_laps.keys()), default=list(selected_laps.keys())[:min(3, len(selected_laps))], key="plotlab_drivers")
        x_axis = st.selectbox("X axis", ["Distance", "TimeSeconds", "LapPercent", "Speed"], index=0, key="plotlab_x")
    with pc2:
        y_channels = st.multiselect("Y channels", available_channels, default=[c for c in ["Speed", "Throttle", "Brake"] if c in available_channels], key="plotlab_y")
        smoothing = st.slider("Smoothing samples", 1, 25, 1, 1, key="plotlab_smooth")
    mode = st.radio("Trace mode", ["lines", "markers", "lines+markers"], horizontal=True, index=0, key="plotlab_mode")
    st.plotly_chart(plot_lab_figure(lap_tels, labels, styles, plot_drivers, x_axis, y_channels, smoothing, mode), use_container_width=True)
...
with tabs[8]:
    ...
    render_html("<div class='f1-card'><div class='f1-card-title'>Engineering readout</div>" + "".join(build_insight_rows(selected_laps, reference_driver, deltas, exit_table, stint_export)) + "</div>")
    st.caption("Insights are derived from public timing/telemetry. They identify where to look; they are not a substitute for confidential team telemetry or run-plan data.")
